package musicquizbot.listener;

import java.util.concurrent.TimeUnit;

import musicquizbot.MusicquizBot;
import musicquizbot.YoutubeRequest;
import musicquizbot.games.Song;
import musicquizbot.games.SongQuiz;
...
public class CommandListener extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (event.isFromType(ChannelType.TEXT)) {
			...
			} else if (!event.getAuthor().isBot() && activeGame != null && activeGame.state == SongQuiz.GameState.GUESSING
		...
		if (event.isFromType(ChannelType.PRIVATE)) {
			...
			} else if (activeGame != null && activeGame.state == SongQuiz.GameState.REQUESTING) {
	...
	public void onMessageReactionAdd(MessageReactionAddEvent event) {
		...
		if (event.isFromType(ChannelType.PRIVATE) && !user.isBot()) {
			if (activeGame != null && activeGame.state == SongQuiz.GameState.REQUESTING
		...
		if (event.isFromType(ChannelType.TEXT) && !user.isBot()) {
			if(activeGame != null && activeGame.state == SongQuiz.GameState.GUESSING
